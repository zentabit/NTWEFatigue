...
from lwe_gen import init_mlwe_lattice
...
def mlwe_kernel(params, seed=None):
    if seed is None:
        params, seed = params
    ...
    C, D = init_mlwe_lattice(n, m, q, d, np.sqrt(sigmasq))
    dual = D @ np.linalg.inv(np.transpose(D) @ D)
    ...
    for blocksize in list(range(2, n * (d + m) + 1)):
